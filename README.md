...
1. **Themen**: Der Bot nutzt **nur aktuelle Google-Trends** – keine `ideas.json` nötig. Es werden Suchtrends aus deinem Land (z. B. Deutschland) geholt; daraus entstehen Fakten-Videos („Fakten über X, die kaum einer kennt“). Themen, die gerade viele suchen, funktionieren oft auch gut auf TikTok.
2. **Video**: **TTS** (Edge oder OpenAI), **synchroner Lauftext**, **Hintergrund** per Pexels (themenpassend) oder lokalem Ordner. Vertikal 1080×1920 (TikTok).
3. **Upload**: Optional per TikTok-API oder manuell in der App (Videos aus `output/`, Übersicht unter :8080).

**Optional:** In `.env` kannst du `IDEA_SOURCE=trends_then_file` setzen (dann Fallback auf `ideas.json`, wenn keine Trends da sind) oder `IDEA_SOURCE=file` (nur eigene Ideen aus `ideas.json`). Standard ist `trends` – nur Trends, keine Datei.
...
## Themen: nur Trends (keine ideas.json nötig)

- **Standard:** Der Bot nutzt **nur Google Trends** – aktuelle Suchtrends aus deinem Land (z. B. Deutschland). Daraus werden Fakten-Videos („Fakten über X, die kaum einer kennt“). **Keine ideas.json nötig.** In `.env`: `IDEA_SOURCE=trends` (Standard), `TRENDS_COUNTRY=germany`, `TRENDS_CACHE_HOURS=12`.
- **Mit Fallback:** `IDEA_SOURCE=trends_then_file` – zuerst Trends, wenn keine da sind, dann `ideas.json`.
- **Nur eigene Ideen:** `IDEA_SOURCE=file` und `ideas.json` befüllen.

**ideas.json** (nur bei `trends_then_file` oder `file`): Jede Idee hat:
...
├── ideas.json           # Optional (nur bei IDEA_SOURCE=file oder trends_then_file)
├── requirements.txt
├── .env.example
